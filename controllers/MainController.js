...
module.exports = {
  ...
  getCardByID: async (req, res) => {
    ...
    `;
      const gatePassResult = await pool.request().query(gatePassQuery);
    ...
    `;
      const violationsResult = await pool.request().query(violationsQuery);
      if (violationsResult.recordset.length > 0) {
        return res.status(200).json({
          source: "Violations",
          data: violationsResult.recordset,
        });
      }
